...
module.exports = {
  signAccessToken: (userId, role) => {
    return new Promise((resolve, reject) => {
      ...
      console.log("Secret token key", secret)
      const options = {
        expiresIn: '1h',
        issuer: 'policyJunction',
        audience: userId
      }
  ...
  verifyTokenWithPermits : (token) =>{
    console.log("verifyTokenWithPermits")
    if (!token) {
  ...
  return new Promise((resolve, reject) => {
    console.log("Fetching secret token", process.env.ACCESS_TOKEN_SECRET)
